/* PRELOADER DISMISSAL & SCROLL REVEAL INITIALIZATION */
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (preloader) {
        setTimeout(function() {
            ...
            // Wait slightly for fade transition, then init reveals
...
AOS.init({
    duration: 680,
    once: true,
    offset: 120 // Increased from 55 to trigger when scrolled deeper into view
});
...
function openMenuPop(card) {
    ...
    document.getElementById('mpAddCart').innerHTML = '<i class="fab fa-whatsapp"></i> Order on WhatsApp';
    document.getElementById('mpAddCart').style.background = 'linear-gradient(135deg, #25D366, #128C7E)';
...
document.getElementById('mpPlus').addEventListener('click', function() {
    ...
    if (currentActiveProduct) {
        currentActiveProduct.qty = mpQty;
    }
});
...
// Add to cart button (Redirects order details straight to WhatsApp)
document.getElementById('mpAddCart').addEventListener('click', function() {
    if (!currentActiveProduct) return;

    // Calculate total price dynamically
    var priceNum = parseFloat(currentActiveProduct.price.replace(/[^0-9.]/g, ''));
    var total = isNaN(priceNum) ? currentActiveProduct.price : '$' + (priceNum * currentActiveProduct.qty).toFixed(2);

    // Format the order template message
    var message = "Hello The Food Hub, I would like to order:\n\n" +
                  "🍔 *Item:* " + currentActiveProduct.title + "\n" +
                  "🔢 *Quantity:* " + currentActiveProduct.qty + "\n" +
                  "💰 *Price (Each):* " + currentActiveProduct.price + "\n" +
                  "🧾 *Total Price:* " + total + "\n\n" +
                  "Please confirm my order details. Thank you!";

    var whatsappUrl = "https://wa.me/918012790715?text=" + encodeURIComponent(message);

    // Open WhatsApp order portal in a new tab
    ...
    setTimeout(function() {
        closeMenuPop();
        self.innerHTML = '<i class="fab fa-whatsapp"></i> Order on WhatsApp';
    }, 1200);
});
...
/* SCROLL REVEAL OBSERVER (Timeline & Reveal Containers) */
function initScrollReveals() {
    var revealElements = document.querySelectorAll('.timeline-reveal, .reveal-container');
    if ('IntersectionObserver' in window) {
        var observer = new IntersectionObserver(function(entries, observer) {
            entries.forEach(function(entry) {
                if (entry.isIntersecting) {
                    entry.target.classList.add('active');
                    // Stop observing once revealed to keep it active
                    observer.unobserve(entry.target);
                }
            });
        }, {
            threshold: 0.15, // Require 15% visibility
            rootMargin: '0px 0px -120px 0px' // Trigger only when element is 120px in view
        });
